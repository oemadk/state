...
//related properties start
	public function getrelatedPropertyName($property_id){
		...
		$sql= "SELECT * FROM " . DB_PREFIX . "property p LEFT JOIN " . DB_PREFIX . "property_description ppd on (p.property_id=ppd.property_id) WHERE p.property_id = '" . (int) $property_id . "'";	
		$query = $this->db->query($sql);
		return $query->row;
	...
	public function getrelatedproperties($category_id){

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "property_to_category WHERE category_id = '" . (int) $category_id . "'limit 3");
